...
extern void readFile2(File file, uint8_t **ppbuf, int *pLen);
...
static esp_err_t playback_handler(httpd_req_t *req)
{
    ...
    const char* _STREAM_PART2 = "Content-Type: image/jpeg\r\nContent-Length: %u\r\n\r\n";
    //
    ...
    while(file)
    {
        ...
        {        
          ...
          if (len > 0 && buf != NULL)
          {
            ...
            if(res == ESP_OK){
              size_t hlen = snprintf((char *)part_buf, 64, _STREAM_PART2, len);
...
static esp_err_t capture_handler(httpd_req_t *req)
{
    ...
    char ts[32];
    snprintf(ts, 32, "%ld.%06ld", fb->timestamp.tv_sec, fb->timestamp.tv_usec);
    Serial.printf("Capture called: %s\n\r", ts);
    httpd_resp_set_hdr(req, "X-Timestamp", (const char *)ts);
...
static esp_err_t stream_handler(httpd_req_t *req)
{
    ...
    if (res != ESP_OK)
    {
        Serial.printf("stream_handler - httpd_resp_set_type failed\n\r");
        return res;
    }
...
void startCameraServer()
{
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK)
    {
        ...
        httpd_register_uri_handler(camera_httpd, &erase_all_uri);
    }
    ...
    {
        Serial.printf("Started stream server on port: '%d'\n\r", config.server_port);
        httpd_register_uri_handler(stream_httpd, &stream_uri);
        httpd_register_uri_handler(stream_httpd, &playback_uri);
    }
}
