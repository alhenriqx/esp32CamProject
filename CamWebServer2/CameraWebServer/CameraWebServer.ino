...
void loop() 
{
    if (snapShotEnabled)
    {
      Serial.printf("Snapshot enabled!!\n\r");
      ...
      if ((millis() - snapShotTimer) > 50)
      {
          ...
          if (snapshot_counter > 29)
          {
              ...
              gpio_isr_handler_add(GPIO_TRIGGER_PIN, &onMotionDetected, NULL);
    ...
    {
      snapShotTimer = millis();
      Serial.printf("delay for 10seconds\n\r");
      delay(10000);
    }
}
...
void IRAM_ATTR onMotionDetected(void *arg) 
{
    if (currentStatus == END)
    {
      ...
      gpio_isr_handler_remove(GPIO_TRIGGER_PIN);
    }
}